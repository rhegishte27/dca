...
class BackupKeepIntervalMapperTest {
	private BackupKeepIntervalMapper mapper;

	@BeforeEach
	void setUp() {
		mapper = new BackupKeepIntervalMapperImpl();
	}

	@Nested
	class ToEntity {
		private BackupKeepIntervalDto dto;
		...
		void givenDtoNull_whenToEntity_returnNull() {
			...
			// when
			BackupKeepInterval actual = mapper.toEntity(dto);
		...
		void givenIdDtoNull_whenToEntity_returnNull() {
			// given
			dto = BackupKeepIntervalDto.builder().build();
		...
		void givenInvalidDtoId_whenToEntity_throwIllegalArgumentException(Integer id) {
			// given
			dto = BackupKeepIntervalDto.builder().id(id).build();
		...
		void givenDtoWithRightId_whenMap_returnProjectWithRightData(Integer id) {
			// given
			dto = BackupKeepIntervalDto.builder().id(id).build();
			BackupKeepInterval expected = BaseEnum.valueOf(BackupKeepInterval.class, id);
